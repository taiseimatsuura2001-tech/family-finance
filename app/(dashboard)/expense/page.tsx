...
import { Plus, TrendingDown, Calendar, Building, CreditCard, FileText, Edit, Trash2, ArrowUpDown } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function ExpensePage() {
  ...
  if (isLoading || isLoadingCategories) {
    return <PageLoading message="支出データを読み込んでいます..." />;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">支出管理</h1>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
          <UserSelector />
          {!isViewingOther && (
            <Link href="/expense/new" className="w-full sm:w-auto">
      ...
      {filteredTransactions.length === 0 ? (
        <Card className="border-2 border-dashed">
          <CardContent className="py-8">
            <div className="text-center">
              <div className="p-3 bg-red-50 rounded-full w-12 h-12 mx-auto mb-3 flex items-center justify-center">
              ...
              <p className="text-xs text-muted-foreground mb-4">
                {isViewingOther ? "このユーザーの支出記録はありません" : "最初の支出を登録して、家計管理を始めましょう"}
              </p>
              {!isViewingOther && (
        ...
        <Card>
          <CardHeader className="border-b bg-gradient-to-r from-red-50 to-transparent py-3 px-4">
          ...
          <CardContent className="p-0">
            <div className="divide-y">
              {filteredTransactions.map((transaction: any) => (
                <div
                  key={transaction.id}
                  className="p-3 hover:bg-red-50/50 transition-colors"
                >
      ...
      <AlertDialog open={!!deletingId} onOpenChange={(open) => !open && setDeletingId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              この操作は取り消せません。本当にこの支出を削除してもよろしいですか？
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
